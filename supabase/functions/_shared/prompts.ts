// The system prompts, duplicated here on purpose.
//
// The client copy in src/lib/ai-prompts.ts is for display — the landing page
// shows it, and the prompt is part of the pitch. This copy is the one that
// actually runs, and it must not be reachable from the browser: a prompt the
// client can rewrite is a prompt an attacker can rewrite.
