  ...
  const exportWebsite = () => {
    ...
    const pages = rawPages.length
      ? rawPages.map((page: any, index: number) => ({
          ...
          title: String(page.title || page.path || `Page ${index + 1}`),
      ...
      .map((page: any) => {
        ...
        const cards = sectionNames
          .map((section: any) => {
            const title = typeof section === 'string'
              ? section
              : String(section?.title || section?.type || 'Section');
            return `<article class="card"><span class="mark">✦</span><h2>${escapeHtml(title)}</h2><p>Editable content generated by DESIGNLY.</p></article>`;
          })
          .join('');
        return `<div class="page" data-path="${escapeHtml(path)}">
          <section class="hero">
            ...
            <button onclick="document.getElementById('contact').scrollIntoView({behavior:'smooth'})">${escapeHtml(design.heroButton || 'CONTACT')}</button>
          </section>
...
<form onsubmit="event.preventDefault();document.getElementById('form-status').textContent='Üzenet elküldve — ez a statikus export demo workflow-ja.'">
<input required placeholder="Neved" aria-label="Neved">
<input required type="email" placeholder="Email" aria-label="Email">
<textarea required placeholder="Üzenet" aria-label="Üzenet"></textarea>
...
(function(){
  ...
  function show(path){
    const target=path || '/';
    let found=false;
    pages.forEach(p=>{const active=p.dataset.path===target;p.classList.toggle('active',active);if(active)found=true});
    links.forEach(a=>a.classList.toggle('active',a.dataset.page===(found?target:'/')));
    if(!found){pages.forEach(p=>p.classList.toggle('active',p.dataset.path==='/'));links.forEach(a=>a.classList.toggle('active',a.dataset.page==='/'))}
  }
