...
export function StreamerStudioPage({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const generate = async () => {
    ...
    const prompt = [
      'DESIGNLY STREAMER GAMER MASTER ASSET',
      ...
      `STYLE: ${style}`,
      `COLORS: ${colors}`,
      `BRIEF: ${brief.trim() || 'Premium streamer/gamer visual identity with strong hierarchy, high contrast, clean safe zones and clear focal point.'}`,
      selected.mode === 'motion-ready' ? 'Build a strong keyframe artwork that can later be animated.' : 'Build a crisp static stream asset ready for export.',
    ].join('\n');
    setGenerating(true);
    setError('');
